...
io.on("connection", (socket) => {
  ...
  if (session && lobbyId && lobbies[lobbyId]) {
    const existingPlayerIndex = lobbies[lobbyId].findIndex(
      (s) => s.request.sessionID === socket.request.sessionID
    );

    if (existingPlayerIndex !== -1) {
      // Player is already in the lobby, ignore the connection
      console.log(
        `Player ${existingPlayerIndex} reconnected to lobby ${lobbyId}`
      );
  ...
  socket.on("join-lobby", (selectedLobbyId) => {
    if (lobbies[selectedLobbyId]) {
      ...
      if (playerIndex !== -1) {
        ...
        io.to(selectedLobbyId).emit("player-connection", {
          playerIndex,
  ...
  socket.on("fire", (id) => {
    console.log(
      `Shot fired from player ${playerIndex} in lobby ${lobbyId}`,
      id
    );
  ...
  function getLobbiesList() {
    return Object.keys(lobbies).map((lobbyId) => {
      const playersCount = lobbies[lobbyId].filter(
        (player) => player !== null
      ).length;
