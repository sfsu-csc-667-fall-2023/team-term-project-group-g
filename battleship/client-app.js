document.addEventListener('DOMContentLoaded', () => {
    ...
    socket.on('update-player-elements', (lobbyPlayers) => {
  // Update HTML elements based on lobbyPlayers array
  lobbyPlayers.forEach((player, index) => {
    const playerElement = document.querySelector(`.player.p${index + 1}`);
    const connectedElement = playerElement.querySelector('.connected');
    const readyElement = playerElement.querySelector('.ready');

    // Update 'connected' element
    connectedElement.classList.toggle('active', player !== null && player !== undefined);

    // Update 'ready' element
    if (player !== null && player !== undefined && player.ready) {
      readyElement.classList.add('active');
    } else {
      readyElement.classList.remove('active');
    }
  });
});
      ...
      computerSquares.forEach(square => {
        square.addEventListener('click', () => {
            if (currentPlayer === 'user' && ready && enemyReady) {
                // Get the id of the square clicked
                const squareId = square.dataset.id;
                
                // Emit the 'fire' event to the server with role information
                socket.emit('fire', { id: squareId, role: currentPlayer });
            }
        });
    })
