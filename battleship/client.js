document.addEventListener('DOMContentLoaded', () => {
  ...
  const senderName = `Player (Lobby ${info.lobbyId})`;
  ...
  joinButton.addEventListener('click', () => {
    ...
    if (selectedLobbyId) {
      ...
      socket.on('player-info', (info) => {
        console.log(`Joined lobby ${info.lobbyId} as player ${info.playerIndex} with role ${playerRole}`);
        const lobbyInfo = document.getElementById('lobby-info');
        lobbyInfo.innerText = `Lobby ID: ${info.lobbyId}`;

        // Update data-player-role attribute for player divs
        updatePlayerRoleStyles();
      });
  ...
  // Function to update data-player-role attribute for player divs
  function updatePlayerRoleStyles() {
    const playerDivs = document.querySelectorAll('.player');
    playerDivs.forEach((playerDiv) => {
      playerDiv.setAttribute('data-player-role', playerRole);
    });
  }
});